...
export default function ExpenseTable(props) {
    ...
    const [columns] = useState([
        ...
        {name: 'end_customer', title: 'Klient końcowy'},
    ]);
    ...
    const [columnOrder, setColumnOrder] = useState(JSON.parse(window.localStorage.getItem('expense_table')) || ['status', 'id', 'name', 'surname', 'internal_invoice_number', 'invoice_number', 'description', 'hours_value', 'hours_value_netto', 'hours_value_gross', 'issue_date', 'payment_date', 'purchaser_extended', 'end_customer']);
    ...
    const setColumnOrderFn = nextOrder => {
        localStorage.setItem("expense_table", JSON.stringify(nextOrder));
    ...
    async function fetchNewInvoices() {
        await pulse.invoices.getAllInvoices({
            page: currentPage + 1
        })
            .then(res => res)
            .then(res => {
