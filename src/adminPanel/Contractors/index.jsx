...
export default function Contractors() {
    ...
    const [columnOrder, setColumnOrder] = useState(JSON.parse(window.localStorage.getItem('contracotrs_table')) || ['id', 'title', 'nip', 'company_name', 'city', 'description', 'regon', 'currency']);
    ...
    const setColumnOrderFn = nextOrder => {
        localStorage.setItem("contracotrs_table", JSON.stringify(nextOrder));
    ...
    async function getContractors() {
        await pulse.contractors.getContractors({
            page: currentPage + 1
        })
            .then(res => res)
            .then(res => {
    ...
    return (
        <>
            ...
            <Paper>
                ...
                {selection.length > 0 && <EditForm contractor_id={rows[selection].id}/>}
            </Paper>
        </>
    );
};